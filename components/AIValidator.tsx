        ...
        <ScrollReveal width="100%" className="max-w-3xl mx-auto">
            <div className="text-center mb-16">
                <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-indigo-500/10 border border-indigo-500/20 mb-6">
                    ...
                    <span className="text-xs font-bold tracking-widest text-indigo-300 uppercase">Nexus AI Validator</span>
                </div>
                <h2 className="text-3xl md:text-5xl font-bold text-white mb-6">
                    Fikrinizi teknik olarak<br />
                    <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 to-purple-400">doğrulayın.</span>
                </h2>
                <p className="text-slate-400 text-lg">
                    Aklınızdaki proje teknolojik olarak ne kadar mümkün? Hangi altyapıyı kullanmalı? Nexus AI saniyeler içinde analiz etsin.
                </p>
            </div>
            ...
            <AnimatePresence mode="wait">
              {!analysis && (
                ...
                >
                    <input 
                        type="text" 
                        placeholder="Örn: Yapay zeka destekli bir hukuk asistanı uygulaması..." 
                ...
                {analysis && (
                    ...
                    >
                             ...
                             <div className="flex flex-col md:flex-row items-start gap-8 md:gap-12">
                                <div className="flex-shrink-0 flex flex-col items-center w-full md:w-auto">
                                   ...
                                   <div className="inline-flex items-center gap-2 px-3 py-1 bg-slate-800 rounded-lg border border-slate-700 mt-2">
                                      ...
                                      <span className="text-xs text-slate-300 font-medium">MVP: <span className="text-white">{analysis.mvpTimeline}</span></span>
                                   </div>
                                </div>
                                
                                <div className="text-center md:text-left flex-1 w-full">
                                   <h3 className="text-xl font-bold text-white mb-3">Teknik Fizibilite Raporu</h3>
                                   ...
                                   <div className="bg-slate-950/50 p-6 rounded-xl border border-white/5 shadow-inner">
                                      <div className="flex items-center gap-2 mb-4 pb-2 border-b border-white/5">
                                        ...
                                        <span className="text-sm font-bold text-white tracking-wide">Karmaşıklık Dağılımı</span>
                                        <span className="text-[10px] text-slate-500 uppercase ml-auto">Teknik Yük</span>
                                      </div>
                                      <ComplexityBar label="Frontend & UX" value={analysis.complexity.frontend} color="bg-blue-500" />
                                      <ComplexityBar label="Backend & Logic" value={analysis.complexity.backend} color="bg-purple-500" />
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                            ...
                            <motion.div variants={sectionVariants} className="flex flex-col gap-4">
                                ...
                                <div className="bg-slate-900/50 border border-white/5 p-4 rounded-2xl flex items-center gap-4">
                                   ...
                                   <div><div className="text-xs text-slate-500 uppercase">Frontend</div><div className="text-slate-200 font-medium">{analysis.recommendedStack.frontend}</div></div>
                                </div>
                                <div className="bg-slate-900/50 border border-white/5 p-4 rounded-2xl flex items-center gap-4">
                                   <div className="p-2 bg-slate-800 rounded-lg text-purple-400"><Cpu size={20} /></div>
                                   <div><div className="text-xs text-slate-500 uppercase">Backend</div><div className="text-slate-200 font-medium">{analysis.recommendedStack.backend}</div></div>
                                </div>
                                <div className="bg-slate-900/50 border border-white/5 p-4 rounded-2xl flex items-center gap-4">
                                   <div className="p-2 bg-slate-800 rounded-lg text-sky-400"><BarChart3 size={20} /></div>
                                   <div><div className="text-xs text-slate-500 uppercase">Infrastructure</div><div className="text-slate-200 font-medium">{analysis.recommendedStack.infrastructure}</div></div>
                                </div>
                            </motion.div>
                            ...
                            <motion.div variants={sectionVariants} className="bg-[#0f0a0a] border border-red-900/30 rounded-3xl p-6 md:p-8">
                                <div className="flex items-center gap-3 mb-6">
                                    ...
                                    <h3 className="text-lg font-bold text-white">Risk Analizi</h3>
                                </div>
                        ...
                        <motion.div variants={sectionVariants} className="bg-slate-900 border border-emerald-500/20 rounded-2xl p-6 flex items-start gap-4">
                             ...
                             <div>
                                <h3 className="text-lg font-bold text-white mb-2">Gelir Modeli Önerisi</h3>
                        ...
                        <motion.div variants={sectionVariants} className="bg-gradient-to-r from-indigo-900/20 to-slate-900 border border-indigo-500/20 rounded-3xl p-8 flex flex-col md:flex-row items-center gap-6 text-center md:text-left">
                            ...
                            <div className="flex-1">
                               <h3 className="text-lg font-bold text-white mb-2">Nexus Stratejik Tavsiye</h3>
                        ...
                        <motion.div variants={sectionVariants} className="flex flex-col sm:flex-row items-center justify-between gap-6 pt-6 border-t border-white/5">
                            ...
                            >
                                <RefreshCw size={16} /> Farklı bir analiz yap
                            </button>
                            
                            <Button size="lg" onClick={() => document.getElementById('contact')?.scrollIntoView({ behavior: 'smooth' })}>
                                Ücretsiz Danışmanlık Al <ArrowRight size={18} className="ml-2" />
                            </Button>
                        </motion.div>
